...
animelist_dropped <- animelist_clean %>% 
  ...
  select(c(dropped : plan_to_watch), c(aired_from_year : premier_year), c(genre_action : genre_yuri))
...
unknown_NA_count <- cbind(unknown_count, NA_count) %>% 
  map_dbl(as.numeric())
colnames(unknown_NA_count) <- c("unknown", "NA")

overall_invalid_count <- unknown_NA_count %>% 
  mutate(overall = unknown + NA)

set.seed(1234)
train_test_split <- initial_split(animelist_dropped, prop = 0.3)
