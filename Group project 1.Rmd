...
animeinfo_raw <- read_csv(here::here("Data/anime.csv")) %>%
...
unique(animeinfo_raw$type)
unique(animeinfo_raw$source)
unique(animeinfo_raw$rating)
...
animelist_clean_long <- animelist_clean %>%
  ...
  filter(binary == 1) %>% 
  dplyr::select(c(mal_id : droprate), binary) %>% 
...
ggcor_anime <- ggcorr(ggcorr_anime_data, geom = "blank", label = TRUE, hjust = 0.75) +
  ...
  scale_colour_manual(values = c('#C8103E', '#001E62')) +
  guides(color = FALSE, alpha = FALSE) +
  labs(
    title = "<b> Strong sign of multicollinearity between anime interaction variables </b><br>
       <span style = 'font-size:12pt'> The <span style='color:#001E62'>positive</span> and <span style='color:#C8103E'>negative </span> correlations of the numerical variables of the Anime dataset</span><br>  ",
    caption = "Source: Kaggle.com"
  ) +
  theme(plot.title.position = "plot",
        plot.title = element_textbox_simple(size=16, vjust = 10),
...
plot1 <- ggplot(data = count_by_genre, aes(x = count, y = fct_reorder(genre, count))) +
  ...
  labs(
    ...
    caption = "Source: Kaggle.com") +
  labs(
    title = "<b> Anime production is spread across many genres </b><br>
       <span style = 'font-size:12pt'> Number of produced Animes per genre for 1918 until 2020 </span><br>  ",
    caption = "Source: Kaggle.com"
  ) +
  theme(plot.title.position = "plot",
        plot.title = element_textbox_simple(size=16, vjust = 10),
...
drop_plot <- ggplot(animelist_dropped,
  ...
  geom_point()+
  labs(title = "Animes dropped across different scores",
       x = "Score",
       y = "Number of animes dropped",
       colour = "")+
...
drop_scaled <- ggplot(drop_scaled_df,
                    aes(x=score, y=dropped, colour=dropped))+
  geom_point()+
  labs(title = "Number of users who dropped each anime<br><span style = 'font-size:12pt'>With respect to each anime's score",
       x = "Score",
       y = "Number of animes dropped",
       colour = "'")+
  theme(axis.title = element_text(face="bold"))+
...
score_droprate_plot <- ggplot(scoredroprate, 
  ...
  geom_point()+
  scale_colour_gradient2(low = "red", mid = "blue", high = "orange", midpoint = 0.1, breaks = breaks)+
  labs(title = "Drop rates for each anime<br><span style = 'font-size:12pt'>With respect to each anime's score",
       x = "Score of anime",
       y = "Drop rates of anime",
       colour = "")+
...
most_members <- animelist_clean_long %>%
  ...
  mutate(genre = str_to_title(str_replace(genre, "_", " ")),
         median = median(members)) 
  

format(summary(member_df), big.mark = ",") # using big.mark to add commas


plotmembers <- ggplot(most_members, 
       aes(x=genre, y=members, colour=genre))+
  ylim(0,200000)+
  geom_hline(yintercept = mean(most_members$median, na.rm = TRUE), colour = "#C8103E", linetype = "dotted", size = 0.75) +
  geom_boxplot(width = 0.5, lwd=0.5, outlier.size=1, outlier.shape=16, notch=FALSE, colour = "#001E62")+
#  scale_colour_manual(values=c('#00429d', '#4167ac', '#668eba', '#8ab6c8', '#b4ded5', '#ffc964', '#f3956d', '#db6468', '#ba3656', '#93003a'))+
  labs(
    title = "<b> There is no significant sign that Anime's members lean towards particular genres </b><br>
       <span style = 'font-size:12pt'> Number of members across Anime genres for 1918 until 2020 </span><br>  ",
    caption = "Source: Kaggle.com"
  ) +
  geom_label(
    label="Median members is: 25249", 
    x = 10, hjust = 0 , y = 25249,
...
watchtime_by_genre <- animelist_clean_long %>% 
  ...
  ungroup() %>% 
  group_by(genre, type) %>% 
  summarise(watch_time_genretype = sum(duration*episodes*completed, na.rm = TRUE)/1e9,
            watch_time_genre = mean(watch_time_genre)/1e9) %>%
  mutate(genre = str_replace(genre, "_", " ")) %>% 
  mutate(genre = str_to_title(genre))
...
plot3 <- ggplot(data = watchtime_by_genre, aes(x = watch_time_genretype, y = fct_reorder(genre, watch_time_genre), fill = type)) +
  geom_col() +
  labs(
    title = "<b> The watchtime of Animes on TV dominates the industry </b><br>
       <span style = 'font-size:12pt'> Watchtime length of Animes in billion minutes per genre from 1918 until 2020 </span><br>  ",
    caption = "Source: Kaggle.com",
    fill = "Type"
  ) +
  scale_fill_manual(values = c('#c8103e', '#a91946', '#8a1e4e', '#691f55', '#451f5b', '#001e62')) +
  theme(plot.title.position = "plot",
        plot.title = element_textbox_simple(size=16, vjust = 10),
        plot.caption.position = "plot",
        plot.caption = element_textbox_simple(size=12, vjust = 1),
        #axis.title.y = element_text(angle = 0, vjust = 0.5,size=14),
        axis.title.y = element_blank(),
        axis.title.x = element_blank(),
        axis.text = element_text(size=10),
...
plot(plot3)
rm(watchtime_by_genre, most_watched)
...
plot4 <- ggplot(rating_by_duration, aes(y = score, x = mean_duration)) +
  geom_point() +
  facet_wrap(~ type, scales = "free") +
  geom_smooth(aes(colour = type), method = "lm", se = FALSE, colour = '#001e62') +
  ylim(0,10) +
  labs(
    title = "<b> An increase in an anime's duration increases its rating </b><br>
       <span style = 'font-size:12pt'> Rating of animes based on the average runtime per type </span><br>  ",
    caption = "Source: Kaggle.com"
  ) +
  xlab(
    "Runtime in minutes"
...
predictions_dropped_all <- predictions_dropped_all %>% 
  ...
  mutate(aboveall = case_when(predicted > droprate ~ "a",
                          TRUE ~ "b")) %>% 
  mutate(above = case_when(predicted > droprate ~ "b",
                           predicted < droprate & type == "TV" ~ "a",
                          TRUE ~ "c"))

# Plot predictions
ggplot(predictions_dropped_all, aes(x = droprate, y = predicted))+
  geom_point(aes(colour = aboveall), alpha = 0.5) +
  geom_smooth(method = "loess", colour = "#001E62", se = F) +
  geom_abline(intercept = 0, slope = 1, colour = "black") +
  scale_colour_manual(values = c('#AFACC9', '#C8103E')) +
  labs(
    title = "<b>Model's droprate underestimation increases in actual droprates</b>
    <br><span style = 'font-size:12pt'>LASSO regression's predicted droprates on actual droprates of animes from 1918-2020",
    x = "Actual droprate",
...
ggplot(predictions_dropped_all, aes(x = droprate, y = predicted)) +
  geom_point(aes(colour = above), alpha = 0.5) +
  scale_colour_manual(values = c('#C8103E', '#AFACC9', '#001E62')) +
  geom_smooth(method = "loess", colour = "#001E62", se = F) +
  geom_abline(intercept = 0, slope = 1, colour = "black") +
  labs(
    title = "<b>TV droprate underestimations are relatively more severe than that of the other distribution channels</b>
    <br><span style = 'font-size:12pt'>LASSO regression's predicted droprates on actual droprates of animes from 1918-2020, faceted by distribution type",
    x = "Actual droprate",
    y = "Predicted droprate") +
  scale_x_continuous(labels = scales::percent) +
  scale_y_continuous(labels = scales::percent) +
...
predictions_score_all <- predictions_score_all %>% 
  ...
  mutate(aboveall = case_when(predicted > score ~ "a",
                          TRUE ~ "b")) %>% 
  mutate(above = case_when(predicted < score ~ "b",
                           predicted > score & type == "TV" ~ "a",
                          TRUE ~ "c"))

#Plot prediction outcomes
ggplot(predictions_score_all, aes(x = score, y = predicted))+
  geom_point(aes(colour = aboveall), alpha = 0.5) +
  scale_colour_manual(values = c('#C8103E', '#AFACC9')) +
  geom_smooth(method = "loess", colour = "#001E62", se = F) +
  geom_abline(intercept = 0, slope = 1, colour = "black", linetype = "dashed") +
  labs(
    title = "<b>Model's score overestimation is particularly severe for low actual scores</b>
    <br><span style = 'font-size:12pt'>LASSO regression's predicted scores on actual scores of animes from 1918-2020",
    x = "Actual score",
    y = "Predicted score") +
  coord_equal() +
  theme(plot.title.position = "plot",
...
ggplot(predictions_score_all, aes(x = score, y = predicted))+
  geom_point(aes(alpha = 0.5, colour = above)) +
  scale_colour_manual(values = c('#C8103E', '#AFACC9', '#001E62')) +
  geom_smooth(method = "loess", colour = "#001E62", se = F) +
  geom_abline(intercept = 0, slope = 1, colour = "black", linetype = "dashed") +
  labs(
    title = "<b>Model's score overestimation is consistent across all distribution types</b>
    <br><span style = 'font-size:12pt'>LASSO regression's predicted scores on actual scores of animes from 1918-2020, faceted by distribution type",
    x = "Actual score",
    y = "Predicted score") +
  facet_wrap(~type, scale = "free") +
  #scale_colour_manual('#c8103e', '#a91946', '#8a1e4e', '#691f55', '#451f5b', '#001e62') +
  theme(plot.title.position = "plot",
        plot.title = element_textbox_simple(size=16, vjust = 10),
        axis.text = element_text(size=10),
        legend.position = "none",
