...
animelist_clean_long <- animelist_clean %>%
  ...
  filter(binary == 1) %>% 
  select(c(mal_id : premier_year), binary) %>% 
...
ggcorr_anime_data <- animelist_clean_long %>% 
  dplyr::select(genre, score, type, episodes, source, duration, rating, popularity, members, favorites, watching, dropped, completed, on_hold, plan_to_watch, aired_from_year)
...
animelist_dropped <- animelist_clean %>% 
  ...
  select(-c(watching : plan_to_watch)) %>% 
...
predictions_dropped_all <- predict(lasso_dropped_all,testing) %>% 
  cbind(.,
        testing %>% select(c(dropped, type))
        )

colnames(predictions_dropped_all) <- c("predicted", "dropped", "type")
predictions_dropped_all <- predictions_dropped_all %>% 
  mutate(error = predicted - dropped)

# Plot predictions
ggplot(predictions_dropped_all, aes(x = dropped, y = predicted))+
  geom_point(aes(colour = type), alpha = 0.5) +
  geom_smooth(method = "loess", colour = "red", se = F) +
  geom_abline(intercept = 0, slope = 1, colour = "black") +
  labs(
    title = "<b>Predicted dropped on Actual dropped (All variables as predictors)</b>",
    x = "Actual dropped",
    y = "Predicted dropped",
    colour = "Type") +
  theme_minimal() +
...
data.frame(
  RMSE = RMSE(predictions_dropped_all$predicted,
              predictions_dropped_all$dropped),
...
animelist_dropped_genres <- animelist_dropped %>% 
  select(dropped, c(genre_action : genre_yuri))
